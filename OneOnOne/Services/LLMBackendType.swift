...
// Note: AIStudio's `LLMBackendConfiguration` (which depends on a `BackendStatus`
// type) is intentionally omitted here — OneOnOne only needs the `LLMBackendType`
// enum to identify a `DiscoveredModel`'s backend for the load balancer. Backend
// health/status is tracked by AIService's own availability flags.
